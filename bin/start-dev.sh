...
if [ ! -d "$API_GATEWAY_DIR" ]; then
    ...
    git clone https://github.com/Feynix2004/API-Premium-Gateway.git "$API_GATEWAY_DIR"
    if [ $? -ne 0 ]; then
...
if [ -f "$COMPOSE_FILE" ]; then
    # 使用临时文件替换API网关路径
    sed "s|context: /Users/feynix/course/API-Premium-Gateway|context: ${API_GATEWAY_DIR}|g" "$COMPOSE_FILE" > "${COMPOSE_FILE}.tmp"
...
RETRIES=60
until docker compose -f $COMPOSE_FILE exec -T rabbitmq rabbitmq-diagnostics -q ping > /dev/null 2>&1 || [ $RETRIES -eq 0 ]; do
    echo -n "."
    RETRIES=$((RETRIES-1))
    sleep 2
done

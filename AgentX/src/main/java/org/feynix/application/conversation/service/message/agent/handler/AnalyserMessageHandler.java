...
import org.feynix.application.conversation.service.message.agent.analysis.AnalyzerMessage;
import org.feynix.application.conversation.service.message.agent.analysis.dto.AnalyzerMessageDTO;
import org.feynix.application.conversation.service.message.agent.event.AgentWorkflowEvent;
...
public class AnalyserMessageHandler  extends AbstractAgentHandler {
    ...
    protected void transitionToNextState(AgentWorkflowContext<?> context) {
        AnalyzerMessageDTO analyzerMessageDTO = (AnalyzerMessageDTO) context.getExtraData(extraAnalyzerMessageKey);
         // todo feynix 问答消息直接 break，任务消息给到下一个状态

package org.feynix.application.usage.service;
...
import org.feynix.domain.product.service.ProductDomainService;
...
public class UsageRecordBusinessInfoService {
    ...
    public UsageRecordBusinessInfoService(ProductDomainService productDomainService,
    ...
    public static class BusinessInfo {
        ...
        public BusinessInfo(String serviceName, String serviceType, String serviceDescription, 
                          String pricingRule, String relatedEntityName) {
    ...
    public Map<String, BusinessInfo> getBatchBusinessInfo(Set<String> productIds) {
        ...
        // 1. 批量查询商品信息
        Map<String, ProductEntity> productMap = productDomainService.getProductsByIds(productIds)
                .stream()
        ...
        if (!modelIds.isEmpty()) {
            modelMap = llmDomainService.getModelsByIds(modelIds)
                    .stream()
        ...
        if (!agentIds.isEmpty()) {
            agentMap = agentDomainService.getAgentsByIds(agentIds)
                    .stream()
    ...
    /** 根据商品和关联实体生成业务信息 */
    private BusinessInfo generateBusinessInfo(ProductEntity product,
                                            Map<String, ModelEntity> modelMap,
    ...
    private String formatModelPricingRule(Map<String, Object> pricingConfig) {
        ...
        if (inputCostObj != null && outputCostObj != null) {
            ...
            return String.format("输入 ¥%.4f/1K tokens，输出 ¥%.4f/1K tokens（基于每百万token ¥%.2f/¥%.2f计算）",
                    inputCostPer1K, outputCostPer1K, inputCostPerMillion, outputCostPerMillion);
        }

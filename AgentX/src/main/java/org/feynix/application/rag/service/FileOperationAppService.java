...
import org.feynix.application.rag.assembler.FileDetailInfoAssembler;
...
public class FileOperationAppService {
    ...
    public Page<DocumentUnitDTO> listDocumentUnits(QueryDocumentUnitsRequest request, String userId) {
        ...
        IPage<DocumentUnitEntity> entityPage = documentUnitDomainService.listDocumentUnits(
                request.getFileId(), userId, request.getPage(), request.getPageSize(), request.getKeyword()
        );
        
        Page<DocumentUnitDTO> dtoPage = new Page<>(
    ...
    public DocumentUnitDTO updateDocumentUnit(UpdateDocumentUnitRequest request, String userId) {
        // 验证语料是否存在
        DocumentUnitEntity existingEntity = documentUnitDomainService.getDocumentUnit(request.getDocumentUnitId(), userId);
        ...
        // 返回更新后的实体
        DocumentUnitEntity updatedEntity = documentUnitDomainService.getDocumentUnit(request.getDocumentUnitId(), userId);

package org.feynix.infrastructure.auth;
...
public class UserAuthInterceptor implements HandlerInterceptor {
    ...
    private static final List<String> EXCLUDED_PATHS = Arrays.asList(
            "/login", "/health", "/register", "/auth/config",
            "/send-email-code", "/verify-email-code", "/get-captcha", 
            "/reset-password", "/send-reset-password-code", 
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        try {
            ...
            if (!JwtUtils.validateToken(token)) {
                logger.warn("认证失败 - Token验证失败: {} {}, token前缀: {}", 
                    method, requestURI, token.length() > 20 ? token.substring(0, 20) + "..." : token);

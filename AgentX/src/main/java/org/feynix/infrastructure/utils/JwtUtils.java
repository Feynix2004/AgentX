...
public class JwtUtils {
    ...
    public static boolean validateToken(String token) {
        ...
        try {
            ...
            logger.debug("Token验证成功: 用户ID={}, 剩余有效时间={}分钟", 
                claims.getSubject(), 
            ...
            logger.warn("Token验证失败 - JWT异常: {}, Token前缀: {}", 
                e.getMessage(), 
                token.length() > 20 ? token.substring(0, 20) + "..." : token);
            return false;
        } catch (Exception e) {
    ...
    public static boolean isTokenExpiringSoon(String token) {
        ...
        try {
            ...
            if (expiringSoon) {
                logger.info("Token即将过期: 用户ID={}, 剩余时间={}分钟", 
                    claims.getSubject(), timeUntilExpiry / (1000 * 60));
            }

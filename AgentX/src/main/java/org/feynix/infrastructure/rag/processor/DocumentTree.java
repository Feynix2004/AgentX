package org.feynix.infrastructure.rag.processor;
...
public class DocumentTree {
    ...
    private List<ProcessedSegment> splitSegmentByParagraphs(ProcessedSegment segment, String titlePath) {
        ...
        if (availableLength <= config.getMinLength()) {
            // 标题太长，直接截断
            String truncated = content.substring(0, Math.min(content.length(), 
                config.getMaxLength() - config.getBufferSize()));
            ProcessedSegment truncatedSegment = new ProcessedSegment(truncated, 
                segment.getType(), segment.getMetadata());
            copySpecialNodes(segment, truncatedSegment);
        ...
        for (String paragraph : paragraphs) {
            ...
            if (currentLength + paragraphLength <= availableLength) {
                ...
                // 当前段落已满，保存并开始新段落
                if (currentContent.length() > 0) {
                    ProcessedSegment newSegment = createSegmentWithTitle(
                        titlePath, currentContent.toString(), segment);
                ...
                if (paragraphLength <= availableLength) {
                    ...
                    // 单个段落超长，截断处理
                    String truncated = trimmedParagraph.substring(0, 
                        availableLength - config.getBufferSize());
                    ProcessedSegment truncatedSegment = createSegmentWithTitle(
                        titlePath, truncated, segment);
                    results.add(truncatedSegment);
                }
            }
        }
        ...
        // 保存最后一个段落
        if (currentContent.length() > 0) {
            ProcessedSegment finalSegment = createSegmentWithTitle(
                titlePath, currentContent.toString(), segment);
        ...
        if (results.isEmpty()) {
            String fallback = content.substring(0, Math.min(content.length(), 
                config.getMaxLength() - config.getBufferSize()));
            ProcessedSegment fallbackSegment = new ProcessedSegment(fallback, 
                segment.getType(), segment.getMetadata());
            copySpecialNodes(segment, fallbackSegment);
    ...
    private ProcessedSegment createSegmentFromNode(HeadingNode node) {
        ...
        ProcessedSegment segment = new ProcessedSegment(fullContent, 
            org.feynix.domain.rag.model.enums.SegmentType.SECTION, null);
        
        // 添加特殊节点
        for (org.feynix.domain.rag.model.SpecialNode specialNode : node.getSpecialNodes()) {
            segment.addSpecialNode(specialNode);
        }
        
        // 递归收集子节点的特殊节点
        collectSpecialNodesFromChildren(node, segment);
        
        return segment;
    }

    /** 递归收集子节点的特殊节点 */
    private void collectSpecialNodesFromChildren(HeadingNode node, ProcessedSegment segment) {
        for (HeadingNode child : node.getChildren()) {
            for (org.feynix.domain.rag.model.SpecialNode specialNode : child.getSpecialNodes()) {
                segment.addSpecialNode(specialNode);
            }
            // 递归处理子节点
            collectSpecialNodesFromChildren(child, segment);
        }
    }
    ...
    private ProcessedSegment createSegmentWithTitle(String titlePrefix, String content, 
        ...
        ProcessedSegment newSegment = new ProcessedSegment(
            fullContent.toString(),
            originalSegment.getType(),
            originalSegment.getMetadata()
        );
        
        copySpecialNodes(originalSegment, newSegment);
        return newSegment;
    }

    /** 复制特殊节点信息 */
    private void copySpecialNodes(ProcessedSegment source, ProcessedSegment target) {
        if (source.getSpecialNodes() != null && !source.getSpecialNodes().isEmpty()) {
            for (org.feynix.domain.rag.model.SpecialNode specialNode : source.getSpecialNodes().values()) {
                target.addSpecialNode(specialNode);
            }
        }
    }

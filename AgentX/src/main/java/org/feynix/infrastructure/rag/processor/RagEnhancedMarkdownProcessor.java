...
/** RAG增强Markdown处理器
 * 
 * 设计模式：装饰器模式 - 包装纯净解析器，提供基础的结构化分段功能 - 使用增强器列表，对每个段落进行RAG相关的增强处理 - 保持接口一致性，调用方无需感知内部实现差异
 * 
 * 处理流程： 1. 委托纯净解析器进行基础Markdown分段 2. 对每个段落按优先级应用相关增强器 3. 返回增强后的段落列表
 * 
 * 适用场景： - RAG文档处理：需要LLM增强的生产环境 - 智能分析：需要将特殊内容转换为搜索友好的文本
 * 
 * @author claude */
...
public class RagEnhancedMarkdownProcessor implements MarkdownProcessor {
    ...
    @Autowired
    public RagEnhancedMarkdownProcessor(PureMarkdownProcessor pureProcessor, List<SegmentEnhancer> enhancers) {
        ...
        log.info("RAG Enhanced Markdown Processor initialized with {} enhancers: {}", enhancers.size(),
    ...
    public List<ProcessedSegment> processToSegments(String markdown, ProcessingContext context) {
        ...
        try {
            ...
            List<ProcessedSegment> enhancedSegments = baseSegments.stream()
                    .map(segment -> enhanceSegment(segment, context)).collect(Collectors.toList());

            // 第3步：应用占位符替换，合并增强内容
            List<ProcessedSegment> finalSegments = enhancedSegments.stream().map(ProcessedSegment::applyEnhancements)
        ...
        try {
            ...
            for (SegmentEnhancer enhancer : enhancers) {
                try {
                    ...
                    if (enhancer.canEnhance(currentSegment)) {
                        log.debug("Applying {} to segment (type: {}, order: {})", enhancer.getType(),
                                currentSegment.getType(), currentSegment.getOrder());
                    ...
                    log.warn("Enhancer {} failed to process segment (type: {}): {}", enhancer.getType(),
                            currentSegment.getType(), e.getMessage());
            ...
            if (enhancedCount > 0) {
                log.debug("Segment enhanced by {} enhancers: type={}, original_length={}, final_length={}",
                        enhancedCount, segment.getType(), segment.getContent().length(),
                        currentSegment.getContent().length());
            }

...
public class SecondarySegmentSplitter {
    ...
    public List<String> splitIfNeeded(String content, String titleContext) {
        ...
        if (fullContent.length() <= maxVectorLength) {
            log.debug("Content length {} <= max length {}, no split needed", 
                    fullContent.length(), maxVectorLength);
            return List.of(fullContent);
        }
        
        log.debug("Content length {} > max length {}, performing smart split", 
                fullContent.length(), maxVectorLength);
    ...
    private boolean isValidSplit(List<String> chunks) {
        ...
        for (String chunk : chunks) {
            ...
            if (chunk.length() < minVectorLength && chunks.size() > 1) {
                log.debug("Chunk too short: {} < {} (and not the only chunk)", 
                        chunk.length(), minVectorLength);
                return false;
            }
        }
    ...
    public String getSplitStatistics(String originalContent, List<String> chunks) {
        ...
        return String.format("Split %d chars into %d chunks. Avg: %d, Min: %d, Max: %d", 
                originalContent.length(), chunks.size(), avgLength, minLength, maxLength);
    }
}
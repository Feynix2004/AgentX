...
import org.feynix.domain.tool.service.state.impl.GithubUrlValidateProcessor;
import org.feynix.domain.tool.model.dto.GitHubRepoInfo;
import org.feynix.infrastructure.config.GitHubProperties;
...
/**
 * 与 GitHub API 交互的服务。
 * 负责从源GitHub仓库下载内容，以及将内容推送到目标GitHub仓库。
 */
@Service
public class GitHubService {
    ...
    public void init() {
        ...
        logger.info("GitHub服务已初始化，目标仓库: {}/{}",
                gitHubProperties.getTarget().getUsername(),
                gitHubProperties.getTarget().getRepoName());
    }

    /**
     * 解析源GitHub URL。如果URL中未指定ref (分支/标签/commit)，
     * 则获取该仓库默认分支的最新commit SHA作为ref。
     * 
     * @param sourceGithubUrl 源GitHub仓库的URL
     * @return GitHubRepoInfo 包含解析后的仓库所有者、名称、ref和仓库内路径
     * @throws IOException                                        如果与GitHub
     *                                                            API通信时发生错误
     * @throws org.feynix.infrastructure.exception.BusinessException 如果URL无效或仓库不可访问
     */
    public GitHubRepoInfo resolveSourceRepoInfoWithLatestCommitIfNoRef(String sourceGithubUrl) throws IOException {
        ...
        if (basicInfo.getRef() == null || basicInfo.getRef().trim().isEmpty()) {
            logger.info("源URL {} 未指定ref，将获取仓库 {}/{} 的默认分支最新commit SHA",
                    sourceGithubUrl, basicInfo.getOwner(), basicInfo.getRepoName());
            ...
            if (defaultBranch == null || defaultBranch.trim().isEmpty()) {
                throw new org.feynix.infrastructure.exception.BusinessException(
                        "无法获取仓库 " + basicInfo.getFullName() + " 的默认分支。");
            }
            String latestCommitSha = repository.getRef("heads/" + defaultBranch).getObject().getSha();
            logger.info("仓库 {}/{} 的默认分支 {} 最新commit SHA为: {}",
                    basicInfo.getOwner(), basicInfo.getRepoName(), defaultBranch, latestCommitSha);
    ...
    public Path downloadRepositoryArchive(GitHubRepoInfo repoInfo) throws IOException {
        ...
        String archiveUrlString = String.format(

...
public class ContainerShutdownListener implements ApplicationListener<ContextClosedEvent> {
    ...
    public ContainerShutdownListener(ContainerDomainService containerDomainService, 
        ...
        try {
            ...
            for (ContainerEntity container : runningContainers) {
                try {
                    ...
                    stoppedCount++;
                    logger.info("已停止容器: {} (Docker ID: {})", 
                              container.getName(), container.getDockerContainerId());
                } catch (Exception e) {
                    failedCount++;
                    logger.error("停止容器失败: {} (Docker ID: {})", 
                               container.getName(), container.getDockerContainerId(), e);
                }
            }

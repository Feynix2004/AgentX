package org.feynix.interfaces.api.portal.payment;
...
import org.feynix.application.payment.service.PaymentAppService;
...
public class PaymentController {
    ...
    public Result<PaymentResponseDTO> createRechargePayment(
            @RequestBody @Validated RechargeRequest request) {
        
        logger.info("接收充值支付请求: amount={}, platform={}, type={}", 
                   request.getAmount(), request.getPaymentPlatform(), request.getPaymentType());
        
        try {
            PaymentResponseDTO response = paymentAppService.createRechargePayment(request);
            logger.info("充值支付创建成功: orderId={}, orderNo={}", 

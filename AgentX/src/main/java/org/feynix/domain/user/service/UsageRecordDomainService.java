package org.feynix.domain.user.service;
...
import org.feynix.domain.user.repository.UsageRecordRepository;
...
public class UsageRecordDomainService {
     ...
     * @return 用量记录列表
     */
    public List<UsageRecordEntity> getUserUsageByTimeRange(String userId, LocalDateTime startTime, LocalDateTime endTime) {
        if (userId == null || userId.trim().isEmpty()) {
            throw new BusinessException("用户ID不能为空");
        }
        ...
        LambdaQueryWrapper<UsageRecordEntity> wrapper = Wrappers.<UsageRecordEntity>lambdaQuery()
                .eq(UsageRecordEntity::getUserId, userId)
                .ge(UsageRecordEntity::getBilledAt, startTime)
                .le(UsageRecordEntity::getBilledAt, endTime)
                .orderByDesc(UsageRecordEntity::getBilledAt);
    ...
    public Page<UsageRecordEntity> getProductUsageHistory(String productId, int pageNum, int pageSize) {
        ...
        LambdaQueryWrapper<UsageRecordEntity> wrapper = Wrappers.<UsageRecordEntity>lambdaQuery()
                .eq(UsageRecordEntity::getProductId, productId)
                .orderByDesc(UsageRecordEntity::getBilledAt);
     ...
     * @return 用量记录分页结果
     */
    public Page<UsageRecordEntity> getUserProductUsageHistory(String userId, String productId, int pageNum, int pageSize) {
        if (userId == null || userId.trim().isEmpty()) {
            throw new BusinessException("用户ID不能为空");
        }
        if (productId == null || productId.trim().isEmpty()) {

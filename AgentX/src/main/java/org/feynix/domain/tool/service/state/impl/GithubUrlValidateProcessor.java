...
public class GithubUrlValidateProcessor implements ToolStateProcessor {
    ...
    /**
     * 解析并验证GitHub URL。
     * 如果验证成功，返回 GitHubRepoInfo 对象。
     * 如果失败，抛出 BusinessException。
     */
        ...
        try {
            ...
            if (pathInRepo != null && !pathInRepo.isEmpty()) {
                ...
                try {
                    ...
                    logger.warn("无法验证路径 {} 在 ref {} (owner={}, repo={}): {}", pathInRepo, effectiveRef, owner, repoName, e.getMessage());
                    throw new BusinessException("GitHub仓库中指定的路径 '" + pathInRepo + "' 在 ref '" + effectiveRef + "' 中不存在或无法访问。");
                }
            }

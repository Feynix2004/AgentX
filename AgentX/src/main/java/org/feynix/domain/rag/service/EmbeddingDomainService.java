    ...
    public List<DocumentUnitEntity> ragDoc(List<String> dataSetId, String question, Integer maxResults, 
        ...
        try {
            ...
            int searchLimit = finalEnableRerank ? 
                Math.max(finalMaxResults * finalCandidateMultiplier, 30) : finalMaxResults;
            
            log.debug("Starting RAG search with params: datasets={}, question='{}', maxResults={}, minScore={}, rerank={}, searchLimit={}", 
            ...
            if (finalEnableRerank && !textSegmentList.matches().isEmpty()) {
                ...
                log.debug("Applied reranking for query: '{}', got {} matches, took {}ms", 
                         question, embeddingMatches.size(), rerankTime);
            } else {
                embeddingMatches = textSegmentList.matches();
                log.debug("Skipped reranking for query: '{}', using {} vector matches", question, embeddingMatches.size());
            }

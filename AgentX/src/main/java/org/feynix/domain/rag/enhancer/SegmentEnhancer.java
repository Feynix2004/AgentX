package org.feynix.domain.rag.enhancer;
...
/** 段落增强器接口
 * 
 * 设计原则： - 单一职责：每个增强器专注处理特定类型的段落 - 职责分离：只做增强处理，不做基础解析 - 可选增强：可以判断是否需要处理某个段落 - 链式处理：支持多个增强器依次处理同一段落
 * 
 * @author claude */
public interface SegmentEnhancer {
    ...
    /** 增强处理段落
     * 
     * 对段落进行RAG相关的增强处理，如： - 代码块转换为自然语言描述 - 表格内容智能分析 - 图片OCR识别 - 公式转换为文本描述 - 内容翻译等
     * 
     * @param segment 原始段落

...
```bash
# 一键部署脚本 - 使用默认配置
curl -fsSL https://raw.githubusercontent.com/feynix/AgentX-2/main/deploy-allinone.sh | bash
```
...
```bash
# 1. 下载配置文件模板
curl -O https://raw.githubusercontent.com/feynix/AgentX-2/main/.env.example
...
docker run -d \
  ...
  ghcr.io/feynix/agentx-2:latest
```
...
# 方式二：从容器中获取
docker run --rm ghcr.io/feynix/agentx-2:latest cat /agentx/.env.example > .env.example
```
...
```bash
# 下载并运行一键部署脚本
curl -fsSL https://raw.githubusercontent.com/feynix/AgentX-2/main/deploy.sh | bash
```
...
```bash
curl -O https://raw.githubusercontent.com/feynix/AgentX-2/main/docker-compose.standalone.yml
```
...
```bash
# 拉取镜像
docker pull ghcr.io/feynix/agentx-2/frontend:latest
docker pull ghcr.io/feynix/agentx-2/backend:latest  
docker pull ghcr.io/feynix/agentx-2/api-gateway:latest
...
docker run -d --name agentx-mcp-gateway \
  --network agentx-network \
  -p 8005:8080 \
  ghcr.io/feynix2004/mcp-gateway:latest
...
docker run -d --name agentx-api-gateway \
  ...
  ghcr.io/feynix/agentx-2/api-gateway:latest
...
docker run -d --name agentx-backend \
  ...
  ghcr.io/feynix/agentx-2/backend:latest
...
docker run -d --name agentx-frontend \
  ...
  ghcr.io/feynix/agentx-2/frontend:latest
```
...
### All-in-One 镜像（推荐）
- `ghcr.io/feynix/agentx-2:latest` - **完整的 AgentX 系统**，包含所有服务

### 分离式镜像
- `ghcr.io/feynix/agentx-2/frontend:latest` - 前端服务（Next.js）
- `ghcr.io/feynix/agentx-2/backend:latest` - 后端服务（Spring Boot）
- `ghcr.io/feynix/agentx-2/api-gateway:latest` - API网关服务
...
services:
  agentx-backend:
    image: ghcr.io/feynix/agentx-2/backend:v1.0.0
```
...
# 删除相关镜像
docker rmi ghcr.io/feynix/agentx-2/frontend:latest
docker rmi ghcr.io/feynix/agentx-2/backend:latest
docker rmi ghcr.io/feynix/agentx-2/api-gateway:latest
```
...
- 源码仓库: https://github.com/feynix/AgentX-2
- 镜像仓库: https://github.com/feynix/AgentX-2/pkgs/container
- 问题反馈: https://github.com/feynix/AgentX-2/issues